...
makeHist <- function(x) {
  ...
  stbl  <- data.table(Measure, Steps)
  
  # Prepare table plot
  tt    <- ttheme_minimal(
              core=list(bg_params = list(fill=c("white","orange"), col=NA),
              fg_params=list(fontface=1)),
              colhead = list(fg_params=list(col="white"),
                             bg_params=list(fill="orange")))
  s <- tableGrob(stbl, theme=tt, rows=NULL)
  
  # Render histogram and summary statistics to screen
  h_s <- suppressMessages(grid.arrange(h,s, nrow=2))
...
makeTimeSeries <- function(x) {
  ...
  l <- ggplot(x, aes(x=interval, y=steps)) +
    ...
    annotate("text", x=835,y=220, 
             label = paste("15-Min Interval with highest average number of steps begins at", maxInterval), 
             colour="red")
